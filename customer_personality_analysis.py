...
df = pd.read_excel("customer_personality.xlsx")
...
date_col = next((col for col in df.columns if 'date' in col.lower() or 'dt' in col.lower()), None)

if date_col:
    df[date_col] = pd.to_datetime(df[date_col], errors='coerce')
    df['time_bucket'] = pd.qcut(df[date_col].rank(method='first'), 10, duplicates='drop')
else:
    df['time_bucket'] = pd.qcut(df.index, 10, duplicates='drop')
...
for col in df.columns:
    if col not in [target, 'time_bucket']:
        try:
...
# Save IV
pd.DataFrame(iv_dict.items(), columns=['Feature', 'IV']) \
  .sort_values(by='IV', ascending=False) \
  .to_csv("iv_values.csv", index=False)

for col in woe_dict:
    df_woe[col] = df_woe[col].map(woe_dict[col])

df_woe = df_woe.apply(pd.to_numeric, errors='coerce').fillna(0)
...
def cross_val(model):
    return (
        cross_val_score(model, X, y, cv=kfold, scoring='accuracy').mean(),
        cross_val_score(model, X, y, cv=kfold, scoring='f1').mean()
    )
...
}).to_csv("final_model_metrics.csv", index=False)
...
vintage.columns = ['Cohort', 'Total Customers', 'Bad Customers', 'Bad Rate']
